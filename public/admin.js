...
<div class="actions">${(!r.autoStart&&["waiting","countdown"].includes(r.status))?`<button class="btn gold" onclick="startRoom('${r.code}')">▶ 開始遊戲</button>`:""}<button class="btn blue" onclick="copyUrl('${r.code}')">複製玩家網址</button><button class="btn green" onclick="copyShare('${r.code}')">複製文字分享</button><button class="btn purple" onclick="showRules('${r.game}')">查看規則</button><button class="btn" onclick="showResults('${r.code}')">📋 對戰紀錄</button><button class="btn red" onclick="delRoom('${r.code}')">刪除房間</button></div></div>`).join(""):"<p>目前沒有房間。</p>"});
